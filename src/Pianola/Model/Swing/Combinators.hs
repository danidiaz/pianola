...
withMenuBar ps liatype@(maybe click toggle -> lastItemAction) = 
      let go firstitem middleitems lastitem = do
             pokel $ menuflat >=> text firstitem >=> click
             forM_ middleitems $ \f -> 
                retryPoke 1 $ replicate 7 $ narrowK $ popupflat >=> text f >=> click
             retryPoke 1 $ replicate 7 $ narrowK $ popupflat >=> text lastitem >=> lastItemAction
